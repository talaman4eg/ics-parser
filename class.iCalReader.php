...
class ICal
{
    ...
    public function initLines($lines)
    {
        ...
        if (stristr($lines[0], 'BEGIN:VCALENDAR') === false) {
            ...
            for ($i = 0; ($i < count($lines) || $nextLine !== ''); $i++) {
                $line = $nextLine;
                if ($i < count($lines)) {
                    $nextLine = $lines[$i];
                } else {
                ...
                if ($this->isContinuation($nextLine)) {
                ...
                if ($currentLine !== '') {
                    $line = $currentLine . substr($line, 1);
                }
                $currentLine = '';
    ...
    public function addCalendarComponentWithKeyAndValue($component, $keyword, $value)
    {
        $attributes = array();
        if (strstr($keyword, ';')) {
            $rawAttributes = explode(';', $keyword);
            $keyword = array_shift($rawAttributes);
        ...
        switch ($component) {
            ...
            case "VEVENT":
                ...
                if($attributes) {
                    if (!isset($this->cal[$component][$this->event_count - 1][$keyword . "_attr"])) {
                        $this->cal[$component][$this->event_count - 1][$keyword . "_attr"] = array();
                    }
                    $last_key = key(array_slice($this->cal[$component][$this->event_count - 1][$keyword . "_array"], -1, 1, TRUE ));
                    $this->cal[$component][$this->event_count - 1][$keyword . "_attr"][$last_key] = $attributes;
                }
                break;
            case "VFREEBUSY":
    ...
    public function isContinuation($line)
    {
        return strlen($line) > 0 && in_array(ord($line[0]), array(9, 32));
    }
